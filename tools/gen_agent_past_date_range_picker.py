# -*- coding: utf-8 -*-
"""Regenerate AgentPastDateRangePicker.vue with UTF-8 Chinese."""
...
T = {
    "start_label": zh(0x5F00, 0x59CB, 0x65F6, 0x95F4),
    "end_label": zh(0x7ED3, 0x675F, 0x65F6, 0x95F4),
    "past": zh(0x8FC7, 0x53BB),
    "today": zh(0x4ECA, 0x5929),
    "day_suffix": zh(0x5929),
    "confirm": zh(0x786E, 0x5B9A),
    "clear": zh(0x6E05, 0x7A7A),
    "placeholder": zh(0x8BF7, 0x9009, 0x62E9, 0x65E5, 0x671F),
}
  ...
  >
    ...
    <div class="agent-past-date-range">
      <div class="agent-past-date-range__row">
        <span class="agent-past-date-range__label">{T['start_label']}</span>
        <el-date-picker
          v-model="draftStart"
          type="date"
          value-format="YYYY-MM-DD"
          placeholder="{T['start_label']}"
          size="small"
          class="agent-past-date-range__picker"
        />
      </div>
      <div class="agent-past-date-range__row">
        <span class="agent-past-date-range__label">{T['end_label']}</span>
        <el-date-picker
          v-model="draftEnd"
          type="date"
          value-format="YYYY-MM-DD"
          placeholder="{T['end_label']}"
          size="small"
          class="agent-past-date-range__picker"
        />
      </div>
      <div class="agent-past-date-range__quick">
        <div class="agent-past-date-range__quick-head">{T['past']}</div>
      ...
      <div class="agent-past-date-range__footer">
        <el-button type="primary" size="small" @click="handleConfirm">{T['confirm']}</el-button>
        <el-button size="small" @click="handleClear">{T['clear']}</el-button>
      </div>
    </div>
  </el-popover>
</template>
...
const quickOptions = [
  ...
  {{ days: 1, label: `1{T['day_suffix']}` }},
  {{ days: 15, label: `15{T['day_suffix']}` }},
  {{ days: 30, label: `30{T['day_suffix']}` }},
  {{ days: 60, label: `60{T['day_suffix']}` }}
]
...
target = (
    ...
    / "frontend/src/views/server/mark/agent/components/AgentPastDateRangePicker.vue"
)

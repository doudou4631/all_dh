<template>
    <div :class="mode == 'pop' ? 'mask' : ''" v-show="showBox">
        <div :class="mode == 'pop' ? 'verifybox' : ''" :style="{ 'max-width': parseInt(imgSize.width) + 30 + 'px' }">
            ...
            <div class="verifybox-bottom" :style="{ padding: mode == 'pop' ? '15px' : '0' }" v-if="componentType">
                <!-- 验证码容器 -->
                <component :is="componentType" :captchaType="captchaType" :type="verifyType" :figure="figure"
                    :arith="arith" :mode="mode" :vSpace="vSpace" :explain="explain" :imgSize="imgSize"
                    :blockSize="blockSize" :barSize="barSize" ref="instance" @ready="onChildReady"
                    @success="onChildSuccess" @error="onChildError" :check="check" />
            </div>
        </div>
    </div>
</template>
...
const verifyType = ref<string | null>(null)
const componentType = shallowRef<typeof VerifySlide | typeof VerifyPoints | null>(null)
...
type INSTANCE = InstanceType<typeof VerifySlide> | InstanceType<typeof VerifyPoints>
const instance = useTemplateRef<INSTANCE>('instance')
...
function onChildSuccess(payload: { captchaVerification: string }) {
    if (props.mode === 'pop') {
        const delay = props.captchaType === 'clickWord' ? 1500 : 0
        if (delay) {
            setTimeout(() => {
                closeBox()
            }, delay)
    ...
    } else if (props.captchaType === 'blockPuzzle') {
        refresh()
    }
    emit('success', payload)
}

const onChildReady = (childProxy: INSTANCE) => emit('ready', childProxy)
const onChildError = (childProxy: INSTANCE) => emit('error', childProxy)

    ...
    <el-card shadow="never" class="baidu-page-card" :body-style="{ padding: '0' }">
      <div class="platform-main">
        ...
        <el-tabs v-model="activeTab" class="sub-tabs">
          <el-tab-pane label="百度 - 提交号码" name="submit">
            <div class="submit-pane">
              <div class="baidu-content-shell">
                <div class="baidu-submit-panel">
                  <div class="baidu-submit-section">
                    <div class="baidu-submit-title">百度标记提交</div>
                    <el-input
                      v-model="phonesText"
                      type="textarea"
                      :rows="9"
                      resize="vertical"
                      class="baidu-submit-textarea"
                      placeholder="可粘贴混合文本，点击「号码提取」整理号码；每行1个号码，7-15位数字"
                    />
                    <div class="baidu-submit-stats">
                      有效号码：<strong>{{ validPhones.length }}</strong> 个
                      <span v-if="validPhones.length > 0" class="baidu-submit-stats__deduct">
                    ...
                    <div class="baidu-submit-actions">
                      ...
                      <div class="baidu-submit-buttons">
                        <el-button
                          type="warning"
                          class="baidu-action-btn baidu-action-btn--extract"
                ...
                <div class="baidu-result-block">
                  <div class="baidu-result-head">提交结果</div>
                  <el-table :data="resultRows" border stripe class="baidu-result-table">
                    ...
                    <template #empty>
                      <span class="baidu-result-empty">暂无提交结果</span>
                    </template>
                  </el-table>
                </div>
              </div>
          ...
          <el-tab-pane label="百度 - 任务记录" name="record">
...
<script setup name="MarkUserBaidu">
...
import { BAIDU_PLATFORM_CODE } from '@/utils/markBaiduPlatform'
...
const queryParams = reactive({
  ...
  keyword: null,
  phone: null,
  platformCode: BAIDU_PLATFORM_CODE,
  orderStatus: null,
  params: {}
})
...
async function loadPlatformInfo() {
  try {
    ...
    const matched = list.find((item) => String(item.platformCode || '').toLowerCase() === BAIDU_PLATFORM_CODE)
...
function getList() {
  queryParams.platformCode = BAIDU_PLATFORM_CODE
...
function exportRecordRows() {
  ...
  downloadCsv(`baidu-record-${Date.now()}.csv`, [header, ...body])
}
...
.baidu-submit-stats strong {
  color: var(--el-color-primary);
  font-weight: 600;
}

.baidu-submit-stats__deduct {
  color: #606266;
}

.baidu-submit-actions {
  display: flex;
  flex-direction: column;
  gap: 8px;
  margin-top: 10px;

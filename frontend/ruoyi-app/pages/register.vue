  ...
  <view class="normal-login-container">
    ...
    <view class="login-form-content">
      ...
      <view class="input-item flex align-center" style="width: 60%;margin: 0px;" v-if="captchaEnabled">
        ...
        <input v-model="registerForm.code" type="number" class="input" placeholder="请输入计算结果" maxlength="8" />
...
<script setup>
  import { getCaptcha, getConfigKey, register } from '@/api/login'
  ...
  // 获取图形验证码
  function getCode() {
    getCaptcha({ captchaType: 'math' }).then(res => {
      const data = res.data || {}
      codeUrl.value = data.originalImageBase64 ? 'data:image/png;base64,' + data.originalImageBase64 : ''
  ...
  async function userRegister() {
    registerForm.value.captcha = captchaEnabled.value ? {
      ...registerForm.value.captcha,
      captchaType: 'math',
      wordList: String(registerForm.value.code || '').trim().split('')
    } : {}
  ...
  getConfigKey('sys.account.captchaEnabled').then(res => {
    captchaEnabled.value = String(res.data || res.msg || '').trim() === 'true'
    if (captchaEnabled.value) {
      getCode()
    }
  }).catch(() => {

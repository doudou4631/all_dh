...
public class VelocityUtils {
    ...
    public static void setTreeVelocityContext(VelocityContext context, GenTable genTable) {
        ...
        if (!paramsObj.get(GenConstants.TREE_PARENT_CODE).isNull()) {
            context.put("tree_parent_code", paramsObj.get(GenConstants.TREE_PARENT_CODE).asText());
        }
        if (!paramsObj.get(GenConstants.TREE_NAME).isNull()) {
    ...
    public static String getParentMenuId(JsonNode paramsObj) {
        if (!paramsObj.get(GenConstants.PARENT_MENU_ID).isNull()
